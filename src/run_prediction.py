...
If no CSV path is provided, the script will prompt the user to enter one.
It will default to dataset/user_test_input.csv when the input is left blank.
...
def main():
    if len(sys.argv) < 2:
        ...
        print("=" * 60)
        default_path = os.path.join(project_root, "dataset", "user_test_input.csv")
        print("\nNo CSV path provided.")
        print(f"Enter the path to your input CSV file, or press Enter to use the default: {default_path}")
        user_csv = input("> ").strip()
        if not user_csv:
            user_csv = default_path
            print(f"Using default input file: {user_csv}")
    else:
        user_csv = sys.argv[1]
